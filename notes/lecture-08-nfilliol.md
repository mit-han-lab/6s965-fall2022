...
### Train from Scratch
- Randomly sampled model architectures can be trained from scratch on the training set.
- The trained model is then evaluated on a validation set to get accuracy $R$.
- Drawback of this simple approach is its prohibitve training cost, as potentially a large amount of GPU hours is required or must be compensated with a smaller design space or training dataset.
- Instead we look for more efficient alternatives for accuracy estimation.
...
### Weight Sharing
Another possible way to reduce the cost of neural architecture search is to use of weight sharing. By training a super-network first and then sampling subgraphs from this network, a model architecture search can be performed. The best-performing sub-network is then retrained from scratch [[Pham *et al.*, ICML 2018]](https://arxiv.org/abs/1802.03268).

![Weight Sharing](figures/lecture-08/weight_sharing.png)

### Performance Estimation Heuristics
Heuristics are used to obtain an estimation of the performance of an architecture with no training at all. An example for such a heuristic ZenNAS [[Lin *et al.*, ICCV 2021]](https://arxiv.org/abs/2102.01063):
...
![Hardware-Aware NAS](figures/lecture-08/hardware_aware_nas.png)
...
![Specialized Models](figures/lecture-08/specialized_models.png)
...
![Once-for-All Netowrk](figures/lecture-08/once_for_all_network.png)

By progressively shrinking different design choises ot the network, the once-for-all network can be steered into learning subnetworks. Several options for progressive shrinking exist such as:
1. Kernel Size
2. Number of Layers
...
### Design Spaces
Both neural architecture but also custom accelerator architecture design are essential to enable specialization and acceleration to make neural networks as efficient as possible. Hardware-Aware Neural Architecture Search as described above only focus on off-the-shelf hardware and thus neglect the freedom in the hardware desing space. Neural Hardware Architecture Search (NHAS) deals with also optimizing design parameters of the accelerator architecture for customized hardware. This results in a huge design space (see below), wherefore a data-driven approach is desireable. 
...
3. Sort the dimensions by the importance value in decreasing order to get the final optimal loop order and/or select the highest the dimensions with the highest importance score for parallelism.
